...
def texture_change(lmdb_path_list, model_path_list):
    ...
    for tex_id in range(30):
        lmdb_path = lmdb_path_list[1]
        model_path = model_path_list[tex_id]
        args.name = 'test'
        ...
        for i in tqdm(range(2)):
            ...
            with torch.no_grad():
                ...
                while True:
                    ...
                    mask_image = pred_image_mask * (1 - bg_batch) + torch.ones_like(align_batch).cuda() * bg_batch
                    # in_im_pred = pred_image_mask + bg_batch * align_batch
                    # pred_image = blender(in_im_pred)

                    pred_img_batch[idx:idx + len(mask_image)] = mask_image.cpu()
                    idx += len(mask_image)
                ...
                os.system("ffmpeg -y -loglevel warning -framerate 25 -start_number 0 -i ../data/tmp/test/%d.png -c:v libx264 -pix_fmt yuv420p -b:v 2000k ../data/tex/{}_{}.mp4".format(tex_id, i))
                os.system("ffmpeg -y -loglevel warning -i ../data/tex/{}_{}.mp4 -i {} -map 0:v -map 1:a -c:v copy -shortest ../data/tex/{}_{}_a.mp4".format(tex_id, i, test_audio_path, tex_id, i))
...
if __name__ == "__main__":
    ...
    #     model_path = "./notun556e_model/{}".format(name)
    ...
    texture_change(lmdb_path_list, model_path_list)