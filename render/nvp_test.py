...
face_unet = nn.DataParallel(model.UNetnop(args.texture_dim, 3).cuda())
blender = nn.DataParallel(model.UNetnop(3, 3).cuda())
tex_sampler.eval()
face_unet.eval()
blender.eval()
texture = None
...
def tune_test(lmdb_path, name, model_path):
    ...
    global args, tex_sampler, face_unet, blender, texture
    args.name = name
    ...
    for i in tqdm(range(35)):
        dst_coeff = np.load("../data/ted_hd/test/{}_7.npy".format(i))
        ...
        with torch.no_grad():
            ...
            os.system("ffmpeg -y -loglevel warning -framerate 25 -start_number 0 -i ../data/tmp/test/%d.png -c:v libx264 -pix_fmt yuv420p -b:v 2000k ../data/nvp/{}_{}.mp4".format(name, i))
            os.system("ffmpeg -y -loglevel warning -i ../data/nvp/{}_{}.mp4 -i {} -map 0:v -map 1:a -c:v copy -shortest ../data/nvp/{}_{}_a.mp4".format(name, i, test_audio_path, name, i))
    ...
    #         for j in range(len(pred_img_batch)):
    #             torchvision.utils.save_image(pred_img_batch[j], "../data/tmp/test/{}.png".format(j), normalize = True, range = (-1, 1))
    #         os.system("ffmpeg -y -loglevel warning -framerate 25 -start_number 0 -i ../data/tmp/test/%d.png -c:v libx264 -pix_fmt yuv420p -b:v 2000k ../data/nvp_re/{}_{}.mp4".format(name, i))
    #         os.system("cp test_align.mp4 ../data/nvp_re/{}_{}_gt.mp4".format(name, i))
...
if __name__ == "__main__":
    ...
    for idx, id_str in enumerate(id_list):
        ...
        model_path = "./nvp_model/{}".format(name)
